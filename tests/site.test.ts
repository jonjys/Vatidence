...
describe("resolveSiteUrl", () => {
  ...
  it("uses the canonical product origin in production when APP_URL is unset", () => {
    expect(resolveSiteUrl(undefined, "production")).toBe("https://viesproof.eu");
  });

  it("refuses to ship localhost in production metadata", () => {
    expect(() => resolveSiteUrl("http://localhost:3000", "production")).toThrow(/localhost/);
    expect(() => resolveSiteUrl("http://127.0.0.1:3000", "production")).toThrow(/localhost/);
  });
